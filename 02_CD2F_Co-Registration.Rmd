...
# Import the CD% data that results from our fitting in LabSpec6
cd_pc_summarized_fitted <- readRDS("cache/area_data.RDS")
```
...
co_registered_fitted <- roi_data_joined %>% 
  left_join(cd_pc_summarized_fitted,
            by = "cell_id") %>% 
  # Exclude cells on the excluded cells list
  filter(!cell_id %in% excluded_cells) %>% 
  # Exclude samples on the excluded samples list
...
saveRDS(co_registered_fitted, "cache/co_registered_fitted.RDS")
```
