...
cd_pc_data <- raman_cell_data %>% 
  ...
  mutate(
    ...
    area = case_when(
      ...
      band == "SR" ~ mean_intensity * (2700 - 2500)
      )
    ) %>% 
  ungroup()
...
cd_pc_data_summarized <- cd_pc_data %>% 
  ...
  mutate(
    # Apply the average silent region noise correction
    CD_corr = CD - SR,
    CH_corr = CH - SR,
    CD_corr = case_when(
      CD_corr < 0 ~ 0,
  ...
  mutate(
    cd_frac = CD / (CH + CD),
    cd_pc = cd_frac * 100,
    cd_frac_corr = CD_corr / (CH_corr + CD_corr),
    cd_pc_corr = cd_frac_corr * 100
    )
...
saveRDS(cd_pc_data_summarized, file = "cache/cd_pc_data_summarized.RDS")
saveRDS(cd_pc_poly_summarized, file = "cache/cd_pc_poly_summarized.RDS")
```


# OPTIONAL: Investigate data with plots
...
## Compare SR corrected values
```{r}
poly_smoothed_plot_corr <- cd_pc_poly_summarized %>% 
  filter(organism != "Mbcal") %>% 
  filter(sample != "Mb_40_24") %>% 
  filter(sample != "Mb_20_23") %>% 
  mutate(f_l = case_when(
  ...
  select(sample, organism, f_l, cell_id, cd_pc, cd_pc_corr) %>%
  pivot_longer(
    cols = c(cd_pc, cd_pc_corr),
    values_to = "cd_percent",
    names_to = "corrected") %>%
  ggplot(
    aes(
      x = as.numeric(f_l),
...
poly_smoothed_plot_corr


```

```{r}
raw_plot_corr <- cd_pc_data_summarized %>% 
  filter(organism != "Mbcal") %>% 
  filter(sample != "Mb_40_24") %>% 
  filter(sample != "Mb_20_23") %>% 
  mutate(f_l = case_when(
...
## Plot typical raman spectrum for each label strength

Create averaged data:
```{r}
cd_pc_data_mn <- cd_pc_data %>% 
  group_by(sample, wavenumber_cm) %>% 
  summarize(
    organism = organism,
    f_l = f_l,
...
cd_pc_data %>% 
  ...
  geom_line(
    data = cd_pc_data_mn %>% filter(sample == "Thy_50_12"),
    color = "red"
    ) +
  labs(
...
cd_pc_data_mn %>% 
  ...
  geom_line() +
  facet_wrap(vars(organism), scales = "free_y") +
