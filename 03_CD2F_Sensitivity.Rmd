...
```{r}
calculate_FT <- function(a = 1, FL, mu, t, F0) {
  e = exp(1) # define euler's number
  return(
    a * FL * (1 - e^(-mu * t)) + F0 * e^(-mu * t)
  )
}
...
p_FT_vs_a <- test_dataset_2 |> 
  ...
  geom_hline(aes(yintercept = aFL, color = as.factor(a)), alpha = 0.3, linetype = "dashed") +
  geom_line() +
  facet_wrap(vars(as.factor(mu)), scales = "free_y") +
  coord_cartesian(xlim = c(0, 80), ylim = c(0, 33), expand = FALSE) +
  scale_color_viridis_d(option = "plasma", begin = 0, end = 0.8) +
  labs(
    x = latex2exp::TeX("\\Delta t"),
    y = latex2exp::TeX("$^{2}F_{biomass}$"),
    color = latex2exp::TeX("$a_{w}$"),
    linetype = latex2exp::TeX("$\\mu$")
  ) +
  theme_bw() +
  theme(
    panel.grid = element_blank(),
...
p_FT_vs_FL <- test_dataset_2 |> 
  ...
  geom_hline(aes(yintercept = aFL, color = as.factor(FL)), alpha = 0.3, linetype = "dashed") +
  geom_line() +
  facet_wrap(vars(as.factor(mu)), scales = "free_y") +
  coord_cartesian(xlim = c(0, 80), ylim = c(0, 40), expand = FALSE) +
  scale_color_manual(values = ggpubfigs::friendly_pal("bright_seven")) +
  labs(
    x = latex2exp::TeX("\\Delta t"),
    y = latex2exp::TeX("$^{2}F_{biomass}$"),
    color = latex2exp::TeX("$F_{L}$"),
    linetype = latex2exp::TeX("$\\mu$")
  ) +
  theme_bw() +
  theme(
...
cowplot::save_plot(
  plot = p_FT_vs_FL,
  filename = "fig_output/p_FT_vs_FL.pdf",
  base_height = 5,
  base_width = 5
)
```
...
```{r}
p_FT_schematic <- test_dataset_2 |> 
  filter(FL == 50, mu == 0.05, !a %in% c(0.1, 0.3, 0.5, 0.9)) |>
  ggplot(
    aes(
      x = t,
      y = FT,
  ...
  coord_cartesian(xlim = c(0, 90), ylim = c(0, 33), expand = FALSE) +
...
cowplot::save_plot(
  plot = p_FT_schematic,
  filename = "fig_output/p_FT_schematic.pdf",
  base_height = 4,
  base_width = 4
)
```
