...
if (file.exists("cache/roi_data.RDS")) {
  ...
  tsv_file_list = list.files(path = "data/nanoSIMS/tsv_output_DEPRECATED/", pattern="*.tsv")
  ...
  for (tsv in tsv_file_list) {
    temp <- read_tsv(paste0("data/nanoSIMS/tsv_output_DEPRECATED/", tsv),
                     col_types = cols())
...
if (file.exists("cache/roi_data_new.RDS")) {
  roi_data_new <- readRDS("cache/roi_data_new.RDS")
} else {
  tsv_file_list = list.files(path = "data/nanoSIMS/tsv_output/", pattern="*.tsv")
  
  roi_data_new <- tibble()
  roi_data_new <- roi_data_new[-1,] # Remove the garbage row of the tibble
  i = 0
  ...
  for (tsv in tsv_file_list) {
    temp <- read_tsv(paste0("data/nanoSIMS/tsv_output/", tsv),
                     col_types = cols())
    temp <- temp %>% mutate(filename = tsv)
    roi_data_new <- bind_rows(roi_data_new, temp)
    i <- i + 1
    print(paste("File", i, "of", length(tsv_file_list), "loaded."))
  }
  saveRDS(roi_data_new, "cache/roi_data_new.RDS")
}
